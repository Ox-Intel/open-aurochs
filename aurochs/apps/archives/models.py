...
class HistoricalEvent(models.Model):
    ...
    def event_data(self):
        if not hasattr(self, "_decrypted_event_data"):
            self._decrypted_event_data = pickle.loads(
                decrypt_bytes(self.encrypted_json_event_data)
            )
    ...
    def log(cls, *args, **kwargs):
        instance_data, event_data = cls.expand_kwargs_to_instance_and_event_data(
            **kwargs
        )
