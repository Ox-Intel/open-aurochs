...
password = settings.AUROCHS_ENCRYPTION_KEY.encode("utf-8")
salt = settings.AUROCHS_ENCRYPTION_SALT.encode("utf-8")
kdf = PBKDF2HMAC(
...
def lookup_hash(s):
    ...
    h.update(
        str("%s%s" % (settings.HASHID_SALT, str(s).strip().lower())).encode("utf-8")
    )
    return h.hexdigest()
...
def create_unique_ox_id(pk, cls, field, hash_length=12):
