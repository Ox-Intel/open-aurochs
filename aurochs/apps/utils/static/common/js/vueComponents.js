Vue.component('editable',{
  ...
  methods:{
    update: function(event){
      // saveSelection();
      aurochs.state.queuedContext = null;
    ...
    keyUp: function(event) {
      try {
        ...
        //   aurochs.state.queuedContext = this;
        //   this.queuedUpdate = setTimeout(function(){
        //     if( event.target.attributes.object != undefined) {
        //       aurochs.state.queuedContext.$emit('update', event.target.attributes.object.value, event.target.attributes.object_key.value, event.target.innerText);
        //     } else {
        //       aurochs.state.queuedContext.$emit('update', event.target.attributes.field_name.value, event.target.innerText);
        //     }
...
Vue.component('page',{
  template:'<div v-bind:class="{\'aurochsPage\':true, \'active\': num == currentpage}" :data-cy="\'aurochsDayPage\' + num" v-bind="$attrs" v-on="$listeners" :id="\'aurochsDayPage\' + num" :style="pageStyle()"><slot></slot></div>',
...
Vue.component('list',{
  template:'<div :data-cy="\'aurochsList_\' + ele_id" v-bind:class="{\'aurochsList\':true, \'big\': big  && big != \'medium\', \'medium\': big == \'medium\', \'bold\': bold, \'bubble\': bubble, \'short\': short, \'has_icon\': icon}" v-bind="$attrs" v-on="$listeners" :style="listStyle()" v-sortable="{ onUpdate: onSort }" :key="component_key" >' +
              ...
              '  </span> ' +
              ' <span class="input_wrapper"><input :big="big" :bold="bold" :data-cy="\'aurochsList_\' + ele_id + \'_newListItem\'" v-on:keyup.enter="enterPressedNewItem" @blur="enterPressedNewItem" v-model="newItemName" @update="newItemAdded"  /></span>' +
    ...
    updateField: function(index, value){
      ...
      if (this.items[index]) {
        this.items[index] = this.items[index] || {
          ...
          "excuse": "",
          "workaround": "",
          "uid": aurochs.methods.generateUID(10),
        };
...
Vue.component('checklist',{
  template:'<div :data-cy="\'aurochsCheckList_\' + ele_id" v-bind:class="{\'aurochsCheckList\':true, }" v-bind="$attrs" v-on="$listeners" :style="listStyle()" v-sortable="{ onUpdate: onSort }" :key="component_key">' +
...
Vue.component('check-button',{
  template: '<div v-bind:class="{\'active\': content, \'aurochsCheckButton\': true, \'button\': true, \'noTitle\': !title}" @click="update">' +
...
Vue.component('star-rating',{
  template: '<div v-bind:class="{\'aurochsStarRating\': true,}" >' +
...
Vue.component('yesnopair',{
  template: '<div class="aurochsYesNoPair"  :data-cy="\'aurochsYesNoPair_\' + ele_id">' + 
...
Vue.component('rotating',{
  template: '<span class="aurochsRotatingMessage">{{ message }}</span>', 
