...
class TestDeleteFramework(EventTestCase):
...
class TestDeleteFrameworkAsTeam(EventTestCase):
...
class TestDeleteFrameworkAsOrg(EventTestCase):
    def test_in_org_deletes(self):
        self.assertEqual(Framework.objects.count(), 0)
        o = Factory.organization()
        o.add_user(self.user)

        r = Factory.framework(organization=o)
        self.assertEqual(Framework.objects.count(), 1)
