...
class TestDeleteReport(EventTestCase):
...
class TestDeleteReportAsTeam(EventTestCase):
...
class TestDeleteReportAsOrg(EventTestCase):
    def test_in_org_deletes(self):
        self.assertEqual(Report.objects.count(), 0)
        o = Factory.organization()
        o.add_user(self.user)

        r = Factory.report(organization=o)
        self.assertEqual(Report.objects.count(), 1)
