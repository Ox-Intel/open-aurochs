...
class TestUpdateFramework(EventTestCase):
    def test_endpoint(self):
        ...
        self.assertEqual(f.subtitle, data["subtitle"])
        ...
        # "criteria_hash": "cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921
        ...
        # 'username': 'zl2E6PrORKYwhVPfmepA50GGOlXzVQrEwHhudh0e0QPmB5beAz7dT6fw6bGPZ1op
        self.assertEqual(user_data["username"], self.user.username)

    def test_update_creating_new_criteria(self):
        f = Factory.framework(user=self.user)
        data = {
            "event_type": "update_framework",
            "id": f.ox_id,
            ...
            "criteria": [
                {
                    ...
                    "weight": Factory.rand_int(start=1, end=10),
                },
                {
                    "name": Factory.rand_str(),
                    "description": Factory.rand_text(),
        ...
        self.resp_body = self.resp_body["objs"]
        self.assertEqual(Framework.objects.all().count(), 1)
        f = Framework.objects.get(pk=f.id)
        self.assertEqual(f.name, data["name"])
        self.assertEqual(f.subtitle, data["subtitle"])

        self.assertEqual(f.criteria.count(), 2)

        obj_data = self.get_nested_obj(f"window.aurochs.data.frameworks['{f.ox_id}']")
        c1, c2 = f.criteria.all()
        self.assertEqual(
            obj_data["criteria"],
            [
    ...
    def test_update_creating_and_removing_criteria(self):
        f = Factory.framework(user=self.user)
        data = {
            "event_type": "update_framework",
            "id": f.ox_id,
        ...
        self.resp_body = self.resp_body["objs"]
        f = Framework.objects.all()[0]
        self.assertEqual(f.criteria.count(), 3)
        ...
        self.resp_body = self.resp_body["objs"]
        f = Framework.objects.all()[0]
        self.assertEqual(f.criteria.count(), 0)

    def test_update_updating_existing_criteria(self):
        f = Factory.framework(user=self.user)
        c = Factory.criteria(framework=f)
        data = {
            "event_type": "update_framework",
            "id": f.ox_id,
            "name": Factory.rand_str(),
        ...
        self.resp_body = self.resp_body["objs"]
        f = Framework.objects.all()[0]
        self.assertEqual(f.criteria.count(), 1)
        c = f.criteria[0]
        self.assertEqual(c.name, data["criteria"][0]["name"])
        self.assertEqual(c.description, data["criteria"][0]["description"])
        self.assertEqual(c.weight, data["criteria"][0]["weight"])
