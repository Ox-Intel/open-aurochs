...
def do_purge():
    ...
    if len(domains) > 0:
        for d in domains:
            ...
            if "result" in r.json() and len(r.json()["result"]) == 1:
                ...
                if not r.status_code == 200:
                    ...
                    print(r.json())
                    raise Exception(
                        "Error purging cache: %s" % r.json()["errors"][0]["message"]
                    )
            else:
                print(r.json())
                print(
                    "Either there isn't a record at cloudflare with domain %s, or there are several."
                    % d
                )
                raise Exception("Cache not purged.")
            print("%s purged" % d)
        ...
        print(
            "No domains specified. Try `python3 purge_cloudflare.py example.com` or similar"
        )
